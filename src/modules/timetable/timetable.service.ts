...
export class TimetableService {
    ...
    async getTimetable(input: TimetableInputQuery): Promise<Timetable[]> {
        return await this.repos._timetableRepository.find({
            where: input,
            order: { dayId: 'ASC', hini: 'ASC' },
        });
    }
    ...
    async createTimetable(input: NewTimetableInput): Promise<boolean> {
        ...
        const exists = await this.repos._timetableRepository.find({
            where: [
                { dayId, companyId, hini: Between(hini, hend) },
        ...
        const savedTimetable: Timetable = await this.repos._timetableRepository.save(
            input,
        );
        if (savedTimetable) {
            return true;
    ...
    async updateTimetable(id: number, input: TimetableInput): Promise<boolean> {
        ...
        const exist = await this.repos._timetableRepository.find({
            where: [
                { dayId, id: Not(id), hini: Between(hini, hend) },
        ...
        const saved = await this.repos._timetableRepository.update(
            { id },
            input,
        );
        if (saved) {
