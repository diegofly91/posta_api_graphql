...
export class TimeEmployeeService {
    ...
    async getTimeEmployee(
        input: TimeEmployeeInputQuery,
    ): Promise<TimeEmployee[]> {
        return await this.repos._timeemployeeRepository.find({
            where: input,
            order: { hini: 'ASC' },
        });
    }
    ...
    async createTimeEmployee(input: NewTimeEmployeeInput): Promise<boolean> {
        ...
        const timeser = await this.repos._timeemployeeRepository.find({
            where: [
                {
                    timetableId,
                    employeeId,
        ...
        const savedTimetable: TimeEmployee = await this.repos._timeemployeeRepository.save(
            input,
        );
        if (savedTimetable) {
            return true;
    ...
    async updateTimeEmployee(
        ...
        const data: TimeEmployee = await this.repos._timeemployeeRepository.findOne(
            { id },
        );
        ...
        const saved = await this.repos._timeemployeeRepository.update(
            { id },
            input,
        );
        if (saved) {
