...
import { TimeEmployeeInput, NewTimeEmployeeInput, TimeEmployeeInputQuery } from './timeemployeeDto/timeemployee.Input';
...
export class TimeEmployeeResolvers {
    constructor(private readonly _timeEmployee: TimeEmployeeService,
                private readonly _tamitableService: TimetableService,) {}

    @Query(() => TimeEmployee, { nullable: true })
    public async getTimeEmployee(@Args('input') input: TimeEmployeeInputQuery): Promise<TimeEmployee[]> {
        return this._timeEmployee.getTimeEmployee(input);
    }
    
    @UsePipes(new ValidationPipe())
    @Mutation(() => TimeEmployee, { nullable: true })
    public async createTimeEmployee( @Args('input') input: NewTimeEmployeeInput): Promise<boolean> {
        return await this._timeEmployee.createTimeEmployee(input);
    }

    @UsePipes(new ValidationPipe())
    @Mutation(() => TimeEmployee, { nullable: true })
    public async updateTimeEmployee( @Args('id') id: number, @Args('input') input: TimeEmployeeInput): Promise<boolean> {
