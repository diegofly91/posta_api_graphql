...
export class TimeServiceService {
    ...
    async getTimeService(input: TimeServiceInputQuery): Promise<TimeService[]> {
        return await this.repos._timeserviceRepository.find({where : input , order:{hini: "ASC"}});
    }
    ...
    async createTimeService(input: NewTimeServiceInput): Promise<boolean> {
        ...
        const timeser = await this.repos._timeserviceRepository.find({where:[{
                                                                            timetableId,
                                                                            serviceId
                                                                    }]});
        if(timeser.length > 0){
        ...
        const savedTimetable: TimeService = await this.repos._timeserviceRepository.save(input);
        if(savedTimetable){
            return true;
        }
        return false;
    ...
    async updateTimeService(id: number,input: TimeServiceInput): Promise<boolean> {
        ...
        const data :TimeService = await this.repos._timeserviceRepository.findOne({id});
        ...
        const saved = await this.repos._timeserviceRepository.update({id},input);
        if(saved){
            return true;
        }
        return false;
