...
export class TimeServiceResolvers {
    constructor(
        private readonly _timeService: TimeServiceService,
        private readonly _tamitableService: TimetableService,
    ) {}

    @Query(() => TimeService, { nullable: true })
    public async getTimeService(
        @Args('input') input: TimeServiceInputQuery,
    ): Promise<TimeService[]> {
    ...
    public async createTimeService(
        @Args('input') input: NewTimeServiceInput,
    ): Promise<boolean> {
    ...
    @UsePipes(new ValidationPipe())
    @Mutation(() => TimeService, { nullable: true })
    public async updateTimeService(
        @Args('id') id: number,
        @Args('input') input: TimeServiceInput,
    ): Promise<boolean> {
